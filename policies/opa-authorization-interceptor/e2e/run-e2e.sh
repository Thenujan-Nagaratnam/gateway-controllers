#!/usr/bin/env bash
#
# One-command runner for the opa-authorization-interceptor e2e suite against a
...
COLLECTION="$ROOT/postman/opa-authorization-interceptor.postman_collection.json"
...
if curl -sf --max-time 1 "http://localhost:$MOCK_PORT/healthz" >/dev/null 2>&1; then
  echo "mock-opa already on :$MOCK_PORT, reusing."
else
  log "Starting mock-opa on :$MOCK_PORT ..."
  (cd "$ROOT/mocks/mock-opa" && ADDR="$MOCK_ADDR" go run . ) >/tmp/mock-opa.log 2>&1 &
  STARTED_PIDS+=("$!")
  wait_healthy "http://localhost:$MOCK_PORT/healthz" "mock-opa"
fi

REGISTERED_PROXIES=(opa-block opa-annotate opa-failopen opa-failclosed opa-err opa-invalid)
cleanup_registered_resources() {
  ...
  curl -s --max-time 25 -o /dev/null -X DELETE "$CONTROLLER_BASE_URL/api/management/v1/llm-providers/opa-e2e-provider" -H "Authorization: Basic YWRtaW46YWRtaW4=" || true
}
cleanup_registered_resources
...
CTXS=(opa-block opa-annotate opa-failopen opa-failclosed opa-err)
...
run_newman "Functional tests" \
  --folder "02 - Allow: request reaches the backend" \
  --folder "03 - Deny: refund over ceiling -> 403" \
  --folder "04 - Allow: small refund -> 200" \
  --folder "05 - Redact obligation applied to the request" \
  --folder "06 - Header-based deny" \
  --folder "07 - onDeny annotate forwards" \
  --folder "08 - Fail-open vs fail-closed when OPA is unreachable" \
  --folder "09 - OPA HTTP 500 -> fail-closed" \
...
if [ "$OVERALL_EXIT" -eq 0 ]; then log "opa-authorization-interceptor e2e suite PASSED"; else log "opa-authorization-interceptor e2e suite FAILED (exit $OVERALL_EXIT)"; fi
exit "$OVERALL_EXIT"
