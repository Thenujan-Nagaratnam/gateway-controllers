#!/usr/bin/env bash
#
# One-command runner for the ai-content-disclosure e2e suite against a
...
COLLECTION="$ROOT/postman/ai-content-disclosure.postman_collection.json"
...
if curl -sf --max-time 1 "http://localhost:$MOCK_PORT/healthz" >/dev/null 2>&1; then
  ...
  (cd "$ROOT/mocks/mock-echo-llm" && ADDR="$MOCK_ADDR" go run . ) >/tmp/mock-echo-llm-disclosure.log 2>&1 &
  STARTED_PIDS+=("$!")
  wait_healthy "http://localhost:$MOCK_PORT/healthz" "mock-echo-llm"
fi

REGISTERED_PROXIES=(disclosure-append disclosure-prepend disclosure-metadata disclosure-invisible disclosure-custom-standard disclosure-min-chars disclosure-invalid)
cleanup_registered_resources() {
  ...
  curl -s --max-time 25 -o /dev/null -X DELETE "$CONTROLLER_BASE_URL/api/management/v1/llm-providers/disclosure-e2e-provider" -H "Authorization: Basic YWRtaW46YWRtaW4=" || true
}
cleanup_registered_resources
...
CTXS=(disclosure-append disclosure-prepend disclosure-metadata disclosure-invisible disclosure-custom-standard disclosure-min-chars)
...
until $ready || [ "$attempt" -ge "$MAX_REGISTER_ATTEMPTS" ]; do
  ...
  ok=true
  for c in "${CTXS[@]}"; do wait_for_route "$c/chat/completions" || { echo "route $c never came up" >&2; ok=false; }; done
  $ok && ready=true
done
...
run_newman "Functional tests" \
  --folder "02 - Default placement (append) adds a visible notice and headers" \
  --folder "03 - placement=prepend adds the notice before the reply" \
  --folder "04 - placement=metadata-only leaves the reply text untouched" \
  --folder "05 - Invisible marker is embedded but not visible" \
  --folder "06 - disclosureStandard is reflected in the header" \
  --folder "07 - minContentChars skips disclosure for very short replies" \
  --folder "08 - Invalid config accepted at registration but the route never discloses" \
  --folder "09 - Cleanup" \
  --reporter-junit-export "$REPORT_DIR/junit.xml" || OVERALL_EXIT=1

if [ "$OVERALL_EXIT" -eq 0 ]; then log "ai-content-disclosure e2e suite PASSED"; else log "ai-content-disclosure e2e suite FAILED (exit $OVERALL_EXIT)"; fi
exit "$OVERALL_EXIT"
