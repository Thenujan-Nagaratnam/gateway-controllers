#!/usr/bin/env bash
#
# One-command runner for the rag-grounding-guardrail e2e suite against a
...
COLLECTION="$ROOT/postman/rag-grounding-guardrail.postman_collection.json"
...
if curl -sf --max-time 1 "http://localhost:$MOCK_PORT/healthz" >/dev/null 2>&1; then
  echo "mock-rag-llm already on :$MOCK_PORT, reusing."
else
  log "Starting mock-rag-llm on :$MOCK_PORT ..."
  (cd "$ROOT/mocks/mock-rag-llm" && ADDR="$MOCK_ADDR" go run . ) >/tmp/mock-rag-llm.log 2>&1 &
  STARTED_PIDS+=("$!")
  wait_healthy "http://localhost:$MOCK_PORT/healthz" "mock-rag-llm"
fi

REGISTERED_PROXIES=(rag-block rag-warn rag-annotate rag-cite rag-specifics rag-invalid)
cleanup_registered_resources() {
  ...
  curl -s --max-time 25 -o /dev/null -X DELETE "$CONTROLLER_BASE_URL/api/management/v1/llm-providers/rag-e2e-provider" -H "Authorization: Basic YWRtaW46YWRtaW4=" || true
}
cleanup_registered_resources
...
CTXS=(rag-block rag-warn rag-annotate rag-cite rag-specifics)
...
run_newman "Functional tests" \
  --folder "02 - Grounded answer passes" \
  --folder "03 - Ungrounded answer is blocked" \
  --folder "04 - Fabricated specifics are flagged" \
  --folder "05 - Warn appends a disclaimer" \
  --folder "06 - Annotate forwards unchanged with headers" \
  --folder "07 - requireCitations" \
  --folder "08 - Short answers are not graded" \
...
if [ "$OVERALL_EXIT" -eq 0 ]; then log "rag-grounding-guardrail e2e suite PASSED"; else log "rag-grounding-guardrail e2e suite FAILED (exit $OVERALL_EXIT)"; fi
exit "$OVERALL_EXIT"
