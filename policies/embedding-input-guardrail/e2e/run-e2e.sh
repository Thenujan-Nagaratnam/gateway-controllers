#!/usr/bin/env bash
#
# One-command runner for the embedding-input-guardrail e2e suite against a
...
COLLECTION="$ROOT/postman/embedding-input-guardrail.postman_collection.json"
...
if curl -sf --max-time 1 "http://localhost:$MOCK_PORT/healthz" >/dev/null 2>&1; then
  ...
  (cd "$ROOT/mocks/mock-echo-llm" && ADDR="$MOCK_ADDR" go run . ) >/tmp/mock-echo-llm-embedguard.log 2>&1 &
  STARTED_PIDS+=("$!")
  wait_healthy "http://localhost:$MOCK_PORT/healthz" "mock-echo-llm"
fi

REGISTERED_PROXIES=(embed-guard-block embed-guard-annotate)
cleanup_registered_resources() {
  ...
  curl -s --max-time 25 -o /dev/null -X DELETE "$CONTROLLER_BASE_URL/api/management/v1/llm-providers/embedguard-e2e-provider" -H "Authorization: Basic YWRtaW46YWRtaW4=" || true
}
cleanup_registered_resources
...
CTXS=(embed-guard-block embed-guard-annotate)
...
until $ready || [ "$attempt" -ge "$MAX_REGISTER_ATTEMPTS" ]; do
  ...
  ok=true
  for c in "${CTXS[@]}"; do wait_for_route "$c/embeddings" || { echo "route $c never came up" >&2; ok=false; }; done
  $ok && ready=true
done
...
run_newman "Functional tests" \
  --folder "02 - Benign input passes" \
  --folder "03 - Dangerous or oversized input is blocked" \
  --folder "04 - onViolation annotate forwards to the backend instead of blocking" \
  --folder "05 - Cleanup" \
  --reporter-junit-export "$REPORT_DIR/junit.xml" || OVERALL_EXIT=1

if [ "$OVERALL_EXIT" -eq 0 ]; then log "embedding-input-guardrail e2e suite PASSED"; else log "embedding-input-guardrail e2e suite FAILED (exit $OVERALL_EXIT)"; fi
exit "$OVERALL_EXIT"
