#!/usr/bin/env bash
#
# One-command runner for the tamper-evident-audit-log e2e suite against a
...
COLLECTION="$ROOT/postman/tamper-evident-audit-log.postman_collection.json"
...
if curl -sf --max-time 1 "http://localhost:$MOCK_PORT/healthz" >/dev/null 2>&1; then
  ...
  (cd "$ROOT/mocks/mock-echo-llm" && ADDR="$MOCK_ADDR" go run . ) >/tmp/mock-echo-llm-tal.log 2>&1 &
  STARTED_PIDS+=("$!")
  wait_healthy "http://localhost:$MOCK_PORT/healthz" "mock-echo-llm"
fi

REGISTERED_PROXIES=(tal-audit tal-invalid)
cleanup_registered_resources() {
  ...
  curl -s --max-time 25 -o /dev/null -X DELETE "$CONTROLLER_BASE_URL/api/management/v1/llm-providers/tal-e2e-provider" -H "Authorization: Basic YWRtaW46YWRtaW4=" || true
}
cleanup_registered_resources

# Probe with the policy's own chain-status marker (not a bare '{}') - this
# policy audits every OTHER request it sees, so a generic readiness probe
# would silently consume real sequence numbers before the functional tests
# ever run and throw off every seq/anchor assertion below.
route_is_up() {
  local code; code=$(curl -sk --max-time 2 -o /dev/null -w '%{http_code}' -X POST "$GATEWAY_URL/$1" -H "Content-Type: application/json" -d '{"auditStatus": true}')
...
until $ready || [ "$attempt" -ge "$MAX_REGISTER_ATTEMPTS" ]; do
  ...
  ok=true
  wait_for_route "tal-audit/chat/completions" || { echo "route tal-audit never came up" >&2; ok=false; }
  $ok && ready=true
done
...
run_newman "Functional tests" \
  --folder "02 - First call chains from the pinned genesis" \
  --folder "03 - Second call links to the first record's hash and is an anchor" \
  --folder "04 - Third call links to the second record and is not an anchor" \
  --folder "05 - Chain-status query reports state without advancing the chain" \
  --folder "06 - Principal and tool identity are recorded via the sink" \
  --folder "07 - Invalid config accepted at registration but the route never audits" \
  --folder "08 - Cleanup" \
  --reporter-junit-export "$REPORT_DIR/junit.xml" || OVERALL_EXIT=1

if [ "$OVERALL_EXIT" -eq 0 ]; then log "tamper-evident-audit-log e2e suite PASSED"; else log "tamper-evident-audit-log e2e suite FAILED (exit $OVERALL_EXIT)"; fi
exit "$OVERALL_EXIT"
