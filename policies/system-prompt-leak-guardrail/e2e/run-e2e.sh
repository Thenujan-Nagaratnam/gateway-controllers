#!/usr/bin/env bash
#
# One-command runner for the system-prompt-leak-guardrail e2e suite against a
...
COLLECTION="$ROOT/postman/system-prompt-leak-guardrail.postman_collection.json"
...
if curl -sf --max-time 1 "http://localhost:$MOCK_PORT/healthz" >/dev/null 2>&1; then
  echo "mock-leaky-llm already on :$MOCK_PORT, reusing."
else
  log "Starting mock-leaky-llm on :$MOCK_PORT ..."
  (cd "$ROOT/mocks/mock-leaky-llm" && ADDR="$MOCK_ADDR" go run . ) >/tmp/mock-leaky-llm.log 2>&1 &
  STARTED_PIDS+=("$!")
  wait_healthy "http://localhost:$MOCK_PORT/healthz" "mock-leaky-llm"
fi

REGISTERED_PROXIES=(spl-block spl-redact spl-annotate spl-nopattern spl-fixed spl-invalid)
cleanup_registered_resources() {
  ...
  curl -s --max-time 25 -o /dev/null -X DELETE "$CONTROLLER_BASE_URL/api/management/v1/llm-providers/spl-e2e-provider" -H "Authorization: Basic YWRtaW46YWRtaW4=" || true
}
cleanup_registered_resources
...
CTXS=(spl-block spl-redact spl-annotate spl-nopattern spl-fixed)
...
run_newman "Functional tests" \
  --folder "02 - Benign reply passes; canary is injected" \
  --folder "03 - System prompt reproduced in reply -> blocked" \
  --folder "04 - Leak-opening pattern; detectPatterns toggle" \
  --folder "05 - Redact removes the canary from the reply" \
  --folder "06 - Annotate forwards with headers" \
  --folder "07 - Fixed canary + configured systemPromptText" \
  --folder "08 - Invalid config accepted at registration but the route never enforces" \
  --folder "09 - Cleanup" \
  --reporter-junit-export "$REPORT_DIR/junit.xml" || OVERALL_EXIT=1

if [ "$OVERALL_EXIT" -eq 0 ]; then log "system-prompt-leak-guardrail e2e suite PASSED"; else log "system-prompt-leak-guardrail e2e suite FAILED (exit $OVERALL_EXIT)"; fi
exit "$OVERALL_EXIT"
