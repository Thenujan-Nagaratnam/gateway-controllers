#!/usr/bin/env bash
#
# One-command runner for the byo-guardrail policy's end-to-end test suite
# (postman/byo-guardrail.postman_collection.json) against a gateway stack
# that is ALREADY running with the byo-guardrail policy built in.
#
# This script does NOT build or start gateway-controller/gateway-runtime
# itself. Since byo-guardrail is registered in gateway/build.yaml via
# `filePath: ./dev-policies/byo-guardrail` (no published module tag yet), the
...
#   2. Starts mock-byo-guardrail-server and mock-echo-llm natively (go run)
#      on fixed ports, unless they're already up.
#   3. Registers the LlmProvider + four LlmProxy variants the collection
#      needs, waits for gateway-runtime to pick each one up via xDS before
#      running the folders that exercise it.
...
COLLECTION="$ROOT/postman/byo-guardrail.postman_collection.json"
...
echo "byo-guardrail was added to build.yaml, LlmProxy registration below will"
...
# mock-byo-guardrail-server doesn't enforce auth in this run (EXPECT_API_KEY
# unset) - byo-main and byo-custom-auth use different credentials/headers
# against the same mock instance, so verification instead reads back what the
# mock actually received via /debug/last-request (see the "06 - Custom auth
# header" folder), which works regardless of which proxy sent the request.
start_mock_if_needed "$ROOT/mocks/mock-byo-guardrail-server" "$BYO_MOCK_ADDR" "$BYO_MOCK_PORT" "mock-byo-guardrail-server"
start_mock_if_needed "$ROOT/mocks/mock-echo-llm" "$ECHO_LLM_MOCK_ADDR" "$ECHO_LLM_MOCK_PORT" "mock-echo-llm"

# --- run newman ----------------------------------------------------------

REGISTERED_PROXIES=(byo-main byo-custom-auth byo-size byo-timeout byo-invalid-no-endpoint byo-invalid-userinfo byo-invalid-scheme byo-invalid-auth)
...
cleanup_registered_resources() {
  ...
  curl -s -o /dev/null -X DELETE "$CONTROLLER_BASE_URL/api/management/v1/llm-providers/byo-guardrail-e2e-provider" \
    -H "Authorization: Basic YWRtaW46YWRtaW4=" || true
}
...
# Poll until the route answers with anything other than 404/000/500 before
# running the folders that exercise it (see guardrails-ai/e2e/run-e2e.sh for
# the same pattern and its notes on an observed environment-level flake in
# this propagation path under rapid multi-proxy registration).
route_is_up() {
  local path="$1"
  local code
...
# the gateway's self-signed dev cert - NODE_TLS_REJECT_UNAUTHORIZED=0 is the
# blunt but reliable fix for local e2e runs against a self-signed cert.
run_newman() {
  local label="$1"; shift
  log "$label"
  NODE_TLS_REJECT_UNAUTHORIZED=0 "${NEWMAN[@]}" run "$COLLECTION" \
    ...
    --env-var "byoMockUrl=localhost:$BYO_MOCK_PORT" \
    --env-var "byoMockPort=$BYO_MOCK_PORT" \
...
ALL_PROXY_CONTEXTS=(byo-main byo-custom-auth byo-size byo-timeout)
...
run_newman "Functional tests" \
  --folder "02 - Request phase: ALLOW and BLOCK" \
  --folder "03 - Response phase: ALLOW and BLOCK" \
  --folder "04 - MODIFY verdict rewrites content transparently" \
...
if [ "$NEWMAN_EXIT" -eq 0 ]; then
  log "byo-guardrail e2e suite PASSED"
else
  log "byo-guardrail e2e suite FAILED (exit $NEWMAN_EXIT)"
fi
