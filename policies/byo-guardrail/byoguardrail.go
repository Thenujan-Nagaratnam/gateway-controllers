...
func GetPolicy(
	...
	// utils.SharedHTTPClient (pooling, config-driven timeouts, and - notably for this
	// policy, since `endpoint` is arbitrary and policy-instance-configured - an
	// optional dial-time SSRF guard) is the preferred pattern for this exact
	// use case, but it isn't in a published sdk/core release yet - using it here
	// would require a local-only go.mod replace directive that can't resolve
	// outside this checkout. Build our own client until a tagged sdk/core release
	// carries it; see the SECURITY NOTE in policy-definition.yaml, which currently
	// relies on validateEndpoint's static scheme/userinfo checks only (no dial-time
	// IP-range guard) until this is revisited.
...
func (p *BYOGuardrailPolicy) evaluate(ctx context.Context, payload []byte, requestID string, params FlowParams, isResponse bool) interface{} {
	...
	case verdictModify:
		if strings.TrimSpace(decision.ModifiedContent) == "" {
			// An empty modifiedContent is a protocol error, not "replace with
			// blank text" - silently rewriting the field to "" and forwarding a
			// 200 would mask a misbehaving guardrail server behind a confusing,
			// silently-emptied response instead of surfacing the failure.
			slog.Warn("BYOGuardrail: MODIFY verdict with empty modifiedContent, treating as error", "isResponse", isResponse)
			if params.PassthroughOnError {
				return passThrough()
			}
			return p.buildErrorResponse(isResponse)
