#!/usr/bin/env bash
#
# One-command runner for the topic-scope-guardrail e2e suite against a
...
COLLECTION="$ROOT/postman/topic-scope-guardrail.postman_collection.json"
...
if curl -sf --max-time 1 "http://localhost:$MOCK_PORT/healthz" >/dev/null 2>&1; then
  ...
  (cd "$ROOT/mocks/mock-echo-llm" && ADDR="$MOCK_ADDR" go run . ) >/tmp/mock-echo-llm-scope.log 2>&1 &
  STARTED_PIDS+=("$!")
  wait_healthy "http://localhost:$MOCK_PORT/healthz" "mock-echo-llm"
fi

REGISTERED_PROXIES=(scope-block scope-annotate scope-response-check scope-invalid)
cleanup_registered_resources() {
  ...
  curl -s --max-time 25 -o /dev/null -X DELETE "$CONTROLLER_BASE_URL/api/management/v1/llm-providers/scope-e2e-provider" -H "Authorization: Basic YWRtaW46YWRtaW4=" || true
}
cleanup_registered_resources
...
CTXS=(scope-block scope-annotate scope-response-check)
...
run_newman "Functional tests" \
  --folder "02 - In-scope and short messages pass through" \
  --folder "03 - Off-topic request is blocked" \
  --folder "04 - Denied topic always blocks regardless of score" \
  --folder "05 - onOffTopic annotate forwards to the backend with a header" \
  --folder "06 - checkResponse blocks a reply that drifts off-topic" \
  --folder "07 - Invalid config accepted at registration but the route never enforces" \
  --folder "08 - Cleanup" \
  --reporter-junit-export "$REPORT_DIR/junit.xml" || OVERALL_EXIT=1

if [ "$OVERALL_EXIT" -eq 0 ]; then log "topic-scope-guardrail e2e suite PASSED"; else log "topic-scope-guardrail e2e suite FAILED (exit $OVERALL_EXIT)"; fi
exit "$OVERALL_EXIT"
