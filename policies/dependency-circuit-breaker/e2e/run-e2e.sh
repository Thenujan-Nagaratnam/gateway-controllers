#!/usr/bin/env bash
#
# One-command runner for the dependency-circuit-breaker e2e suite against a
...
COLLECTION="$ROOT/postman/dependency-circuit-breaker.postman_collection.json"
...
if curl -sf --max-time 1 "http://localhost:$MOCK_PORT/healthz" >/dev/null 2>&1; then
  ...
  (cd "$ROOT/mocks/mock-echo-llm" && ADDR="$MOCK_ADDR" go run . ) >/tmp/mock-echo-llm-depcb.log 2>&1 &
  STARTED_PIDS+=("$!")
  wait_healthy "http://localhost:$MOCK_PORT/healthz" "mock-echo-llm"
fi

cleanup_registered_resources() {
  curl -s --max-time 20 -o /dev/null -X DELETE "$CONTROLLER_BASE_URL/api/management/v1/llm-proxies/dep-circuit-breaker" -H "Authorization: Basic YWRtaW46YWRtaW4=" || true
  curl -s --max-time 25 -o /dev/null -X DELETE "$CONTROLLER_BASE_URL/api/management/v1/llm-providers/depcb-e2e-provider" -H "Authorization: Basic YWRtaW46YWRtaW4=" || true
}
cleanup_registered_resources
...
until $ready || [ "$attempt" -ge "$MAX_REGISTER_ATTEMPTS" ]; do
  ...
  wait_for_route "dep-circuit-breaker/mcp" && ready=true || echo "route dep-circuit-breaker never came up" >&2
done
$ready || { echo "route never came up" >&2; OVERALL_EXIT=1; }
sleep 3

run_newman "Closed-state and threshold tests" \
  --folder "02 - Closed state: repeated success never trips the breaker" \
  --folder "03 - Closed to open after failureThreshold consecutive failures" \
  --folder "04a - Trip breakers for the half-open scenarios (before cooldown)" \
  --reporter-junit-export "$REPORT_DIR/junit-part-a.xml" || OVERALL_EXIT=1

# openDurationSeconds is configured as 1s for this proxy; a Postman
# pre-request script can't reliably sleep through a real wall-clock gap, so
# the half-open assertions run as a second newman invocation after a bash
# sleep - same pattern used by agentic-loop-budget-governor's
# maxDurationSeconds test.
log "Sleeping past openDurationSeconds (1s configured) before the half-open trial calls ..."
sleep 2

run_newman "Half-open trial tests" \
  --folder "04b - Half-open trial after cooldown resolves the breaker" \
  --folder "05 - Cleanup" \
  --reporter-junit-export "$REPORT_DIR/junit-part-b.xml" || OVERALL_EXIT=1

if [ "$OVERALL_EXIT" -eq 0 ]; then log "dependency-circuit-breaker e2e suite PASSED"; else log "dependency-circuit-breaker e2e suite FAILED (exit $OVERALL_EXIT)"; fi
exit "$OVERALL_EXIT"
