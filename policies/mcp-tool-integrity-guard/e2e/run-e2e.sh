#!/usr/bin/env bash
#
# One-command runner for the mcp-tool-integrity-guard e2e suite against a
...
COLLECTION="$ROOT/postman/mcp-tool-integrity-guard.postman_collection.json"
...
if curl -sf --max-time 1 "http://localhost:$MOCK_PORT/healthz" >/dev/null 2>&1; then
  ...
  (cd "$ROOT/mocks/mock-echo-llm" && ADDR="$MOCK_ADDR" go run . ) >/tmp/mock-echo-llm-mcpintegrity.log 2>&1 &
  STARTED_PIDS+=("$!")
  wait_healthy "http://localhost:$MOCK_PORT/healthz" "mock-echo-llm"
fi

REGISTERED_PROXIES=(mcp-integrity-strip mcp-integrity-block mcp-integrity-annotate mcp-integrity-discover)
cleanup_registered_resources() {
  ...
  curl -s --max-time 25 -o /dev/null -X DELETE "$CONTROLLER_BASE_URL/api/management/v1/llm-providers/mcpintegrity-e2e-provider" -H "Authorization: Basic YWRtaW46YWRtaW4=" || true
}
cleanup_registered_resources
...
CTXS=(mcp-integrity-strip mcp-integrity-block mcp-integrity-annotate mcp-integrity-discover)
...
run_newman "Functional tests" \
  --folder "02 - Strip mode: mismatched and unknown tools are removed, matching tool survives" \
  --folder "03 - Block mode: a hash mismatch fails the whole response" \
  --folder "04 - Annotate mode: catalog forwarded unmodified, violations flagged only" \
  --folder "05 - Discover mode: never strips or blocks, reports hashes only" \
  --folder "06 - Cleanup" \
  --reporter-junit-export "$REPORT_DIR/junit.xml" || OVERALL_EXIT=1

if [ "$OVERALL_EXIT" -eq 0 ]; then log "mcp-tool-integrity-guard e2e suite PASSED"; else log "mcp-tool-integrity-guard e2e suite FAILED (exit $OVERALL_EXIT)"; fi
exit "$OVERALL_EXIT"
