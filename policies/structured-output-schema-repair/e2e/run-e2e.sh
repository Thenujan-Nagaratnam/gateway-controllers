#!/usr/bin/env bash
#
# One-command runner for the structured-output-schema-repair e2e suite against a
...
COLLECTION="$ROOT/postman/structured-output-schema-repair.postman_collection.json"
...
if curl -sf --max-time 1 "http://localhost:$MOCK_PORT/healthz" >/dev/null 2>&1; then
  echo "mock-schema-llm already on :$MOCK_PORT, reusing."
else
  log "Starting mock-schema-llm on :$MOCK_PORT ..."
  (cd "$ROOT/mocks/mock-schema-llm" && ADDR="$MOCK_ADDR" go run . ) >/tmp/mock-schema-llm.log 2>&1 &
  STARTED_PIDS+=("$!")
  wait_healthy "http://localhost:$MOCK_PORT/healthz" "mock-schema-llm"
fi

REGISTERED_PROXIES=(sosr-block sosr-passthrough sosr-empty sosr-nodeterm sosr-llm sosr-invalid)
cleanup_registered_resources() {
  ...
  curl -s --max-time 25 -o /dev/null -X DELETE "$CONTROLLER_BASE_URL/api/management/v1/llm-providers/sosr-e2e-provider" -H "Authorization: Basic YWRtaW46YWRtaW4=" || true
}
cleanup_registered_resources
...
CTXS=(sosr-block sosr-passthrough sosr-empty sosr-nodeterm sosr-llm)
...
run_newman "Functional tests" \
  --folder "02 - Clean structured output passes untouched" \
  --folder "03 - Deterministic repair: fences" \
  --folder "04 - Deterministic repair: trailing comma + prose" \
  --folder "05 - Unrepairable -> 502 (onFail block)" \
  --folder "06 - Unrepairable -> passthrough" \
  --folder "07 - Unrepairable -> emptyObject" \
  --folder "08 - deterministicRepair disabled -> fenced output fails" \
  --folder "09 - LLM repair pass" \
...
if [ "$OVERALL_EXIT" -eq 0 ]; then log "structured-output-schema-repair e2e suite PASSED"; else log "structured-output-schema-repair e2e suite FAILED (exit $OVERALL_EXIT)"; fi
exit "$OVERALL_EXIT"
