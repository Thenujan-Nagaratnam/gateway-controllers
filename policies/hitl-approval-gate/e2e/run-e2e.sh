#!/usr/bin/env bash
#
# One-command runner for the hitl-approval-gate e2e suite against a gateway
# stack ALREADY running with the policy built in (both gateway-runtime AND
# gateway-controller must have been rebuilt to pick up a new policy).
#
set -uo pipefail

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
COLLECTION="$ROOT/postman/hitl-approval-gate.postman_collection.json"
...
if curl -sf --max-time 1 "http://localhost:$MOCK_PORT/healthz" >/dev/null 2>&1; then
  ...
  (cd "$ROOT/mocks/mock-echo-llm" && ADDR="$MOCK_ADDR" go run . ) >/tmp/mock-echo-llm-hitl.log 2>&1 &
  STARTED_PIDS+=("$!")
  wait_healthy "http://localhost:$MOCK_PORT/healthz" "mock-echo-llm"
fi

REGISTERED_PROXIES=(hitl-gate hitl-invalid)
cleanup_registered_resources() {
  ...
  curl -s --max-time 25 -o /dev/null -X DELETE "$CONTROLLER_BASE_URL/api/management/v1/llm-providers/hitl-e2e-provider" -H "Authorization: Basic YWRtaW46YWRtaW4=" || true
}
cleanup_registered_resources
...
until $ready || [ "$attempt" -ge "$MAX_REGISTER_ATTEMPTS" ]; do
  ...
  ok=true
  wait_for_route "hitl-gate/mcp" || { echo "route hitl-gate never came up" >&2; ok=false; }
  $ok && ready=true
done
...
run_newman "Functional tests" \
  --folder "02 - Low-risk tool calls pass straight through" \
  --folder "03 - High-risk call blocks pending approval, backend never reached" \
  --folder "04 - Decision submission requires the approver secret" \
  --folder "05 - Approve then retry forwards to the backend" \
  --folder "06 - Deny blocks the retry permanently" \
  --folder "07 - Approval is scoped to the exact arguments reviewed" \
  --folder "08 - Notify webhook fires with the pending approval details" \
...
if [ "$OVERALL_EXIT" -eq 0 ]; then log "hitl-approval-gate e2e suite PASSED"; else log "hitl-approval-gate e2e suite FAILED (exit $OVERALL_EXIT)"; fi
exit "$OVERALL_EXIT"
