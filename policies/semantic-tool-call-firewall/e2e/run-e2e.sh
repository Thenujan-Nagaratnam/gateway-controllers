#!/usr/bin/env bash
#
# One-command runner for the semantic-tool-call-firewall e2e suite against a
...
COLLECTION="$ROOT/postman/semantic-tool-call-firewall.postman_collection.json"
...
if curl -sf --max-time 1 "http://localhost:$MOCK_PORT/healthz" >/dev/null 2>&1; then
  echo "mock-mcp-tool-server already on :$MOCK_PORT, reusing."
else
  log "Starting mock-mcp-tool-server on :$MOCK_PORT ..."
  (cd "$ROOT/mocks/mock-mcp-tool-server" && ADDR="$MOCK_ADDR" go run . ) >/tmp/mock-mcp-tool-server.log 2>&1 &
  STARTED_PIDS+=("$!")
  wait_healthy "http://localhost:$MOCK_PORT/healthz" "mock-mcp-tool-server"
fi

REGISTERED_PROXIES=(stcf-db stcf-http stcf-email stcf-refund stcf-deny-default stcf-annotate stcf-invalid)
cleanup_registered_resources() {
  ...
  curl -s --max-time 25 -o /dev/null -X DELETE "$CONTROLLER_BASE_URL/api/management/v1/llm-providers/stcf-e2e-provider" -H "Authorization: Basic YWRtaW46YWRtaW4=" || true
}
cleanup_registered_resources
...
CTXS=(stcf-db stcf-http stcf-email stcf-refund stcf-deny-default stcf-annotate)
...
run_newman "Functional tests" \
  --folder "02 - SQL statement firewall" \
  --folder "03 - URL SSRF firewall" \
  --folder "04 - Recipient domain allowlist" \
  --folder "05 - Numeric ceiling" \
  --folder "06 - defaultAction block" \
  --folder "07 - annotate mode forwards with headers" \
...
if [ "$OVERALL_EXIT" -eq 0 ]; then log "semantic-tool-call-firewall e2e suite PASSED"; else log "semantic-tool-call-firewall e2e suite FAILED (exit $OVERALL_EXIT)"; fi
exit "$OVERALL_EXIT"
