...
func GetPolicy(
	...
	// TLS, SSRF-guard dial behavior - see utils.SharedHTTPClient's own doc). A nil
	// return means the engine hasn't installed one yet, which is a configuration
	// error, not something to paper over with a locally-built http.Client that would
	// bypass those hardened defaults.
	httpClient := utils.SharedHTTPClient()
	if httpClient == nil {
		return nil, fmt.Errorf("guardrails-ai: shared outbound HTTP client not initialized")
	}
