...
func TestOnRequestBody_NoSuspendUsesGroupsOwnPrimary(t *testing.T) {
	...
	if mods.UpstreamName == nil || *mods.UpstreamName != modelFailoverGroupUpstreamName(p.routeName, "gpt-4o") {
...
// groups) through modelFailoverGroupUpstreamName, pointing UpstreamName at a cluster that
...
func TestOnRequestBody_SuspendedPrimarySkipsAhead(t *testing.T) {
	...
	if mods2.UpstreamName == nil || *mods2.UpstreamName != modelFailoverGroupUpstreamName(p.routeName, "claude-3-5-sonnet") {
		t.Errorf("expected claude's own group to be entirely unaffected by gpt-4o's suspend, got UpstreamName %v", mods2.UpstreamName)
	}
}

func TestModelFailoverGroupUpstreamName_IsRouteScoped(t *testing.T) {
	a := modelFailoverGroupUpstreamName("POST|/chat/completions|main.local", "gpt-4o")
	b := modelFailoverGroupUpstreamName("POST|/embeddings|main.local", "gpt-4o")
	if a == b {
		t.Fatalf("expected same model on different routes to get distinct synthetic upstream names, got %q", a)
	}
	if got := modelFailoverGroupUpstreamName("", "gpt-4o"); got != "__modelfailover_target__gpt-4o" {
		t.Fatalf("empty route name should preserve the historical test/helper formula, got %q", got)
	}
}
...
// TestOnUpstreamAttemptRequestHeaders_RewritesModelPerAttempt locks in the actual kernel
...
func TestOnUpstreamAttemptRequestHeaders_UsesSelectedGroupNotJustAnyGroup(t *testing.T) {
	p := twoGroupPolicy()
	actx := &policy.UpstreamAttemptContext{
		AttemptCount: 2,
		Body:         &policy.Body{Content: []byte(`{"model":"claude-3-5-sonnet","messages":[]}`), Present: true},
	...
	action := p.OnUpstreamAttemptRequestHeaders(context.Background(), actx)
	mods := action.(policy.UpstreamAttemptHeaderModifications)
	var decoded map[string]interface{}
	json.Unmarshal(mods.Body, &decoded)
...
func TestOnUpstreamAttemptRequestHeaders_UnknownBaselineModelFailsOpen(t *testing.T) {
	p := twoGroupPolicy()
	actx := &policy.UpstreamAttemptContext{
		AttemptCount: 2,
...
func TestOnUpstreamAttemptRequestHeaders_AttemptBeyondChainFailsOpen(t *testing.T) {
	p := twoGroupPolicy()
