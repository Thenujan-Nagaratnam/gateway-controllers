...
title: "Overview"
---
# OAuth2 Upstream Authentication
...
entries for the same API.

> **Security note:** `grantType` and `scope` are not part of the key, and
> neither is any hash of `clientId`/`clientSecret`/`password`. A live
> redeploy that changes any of these on the same API can therefore serve a
> still-valid cached token that was minted under the *previous*
> configuration - e.g. a token obtained via `client_credentials` continuing
> to be served after redeploying with `grantType: password`, or a token
> scoped to the old `scope` value being served after `scope` changes. This is
> not harmless: a `client_credentials` token represents the gateway's own
> service identity, while a `password`-grant token represents the resource
> owner's identity, so serving one in place of the other after a grant
> change is a real identity/authorization mismatch, not a benign cache hit.
> Operators changing `grantType`, `scope`, `clientId`, or
> `clientSecret`/`password` on an existing API should not assume the new
> configuration takes effect until the old cache entry's TTL expires. A
> later version of this policy folds these values into the cache key itself
> so a configuration change is guaranteed to miss the old entry - see that
> version's docs.
...
> **Collision risk in the final fallback:** falling back to the route's own
> name is not collision-resistant across policy instances that share a route
> but carry different `oauth2` credentials - the exact shape of an
> `LlmProxy`'s primary provider and an `additionalProviders` entry both
> attached to the same route. In that scenario this fallback can resolve to
> the *same* key for two instances with entirely different credentials,
> letting one provider's cached token be served to another provider's
> backend. Treat this fallback tier as a known gap, not a safe default; a
> later version of this policy removes API/route identity from the key
> entirely in favor of a hash of the oauth2 configuration itself, which
> cannot collide this way - see that version's docs.

The Redis TTL on each entry is derived from the token's own `expires_in` —
the cache never outlives the token it holds.

> **Upgrading from v0.4:** the cache-key schema changed from
> `<keyPrefix><apiId>:<routeName>:<grantType>` (v0.4) to `<keyPrefix><apiId>`
> (this version), while `redis.keyPrefix` keeps the same default
> (`oauth2:token:v1:`) in both. During a mixed-version rollout - some
> `gateway-runtime` replicas still on v0.4, others already on this version -
> replicas compute different keys for what is logically the same cache
> target and so won't share entries with each other until the rollout
> completes. That's a temporary drop in cache-hit rate, not a correctness
> problem: each version's own entries still age out via their normal TTL, so
> no manual Redis flush is required. Operators who want a clean cutover
> instead of that transition window can set a distinct `redis.keyPrefix` as
> part of the upgrade.
...
```yaml
- name: oauth2
  gomodule: github.com/wso2/gateway-controllers/policies/oauth2-upstream-authentication@v0
```
