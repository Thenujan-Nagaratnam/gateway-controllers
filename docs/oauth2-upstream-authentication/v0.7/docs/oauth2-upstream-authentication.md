...
route/resource. This is deliberate: `oauth2` config lives under this
policy's own `params` in `spec.policies`, one value for the whole API —
every resource an `LlmProvider`/`LlmProxy` exposes (`/chat/completions`,
`/embeddings`, ...) is configured with the exact same
`tokenEndpoint`/`clientId`/`clientSecret`, so they should all share the
exact same cached token rather than each independently fetching and caching
its own. Keying by route as well would mint one redundant token (and one
redundant token-endpoint call) per resource instead of one per API.

`grantType` is intentionally **not** part of the key, and neither are
`clientId`/`clientSecret`/`tokenEndpoint`/`params`: there is exactly one
`oauth2` config per API, so none of them can disambiguate two entries for
the same API.

> **Security note:** because none of the above are part of the key, a live
> redeploy that changes `grantType`, credentials, `tokenEndpoint`, or
> `params` (e.g. `scope`) on the same API can serve a still-valid cached
> token minted under the *previous* configuration until that entry's TTL
> expires - this is not harmless in general. A `client_credentials` token
> represents the gateway's own service identity while a `password`-grant
> token represents the resource owner's, so reusing one across a grant
> change is a real identity/authorization mismatch, not a benign cache hit;
> reusing a token minted under an old `tokenEndpoint`/`scope` can likewise
> hand out access the new configuration never actually requested. Operators
> changing any of these on an existing API should not assume the change
> takes effect before the old entry's TTL expires. A later version of this
> policy folds these values into the cache key itself so a configuration
> change is guaranteed to miss the old entry - see that version's docs.
