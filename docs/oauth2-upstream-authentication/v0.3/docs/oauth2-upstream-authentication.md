...
> **Breaking change from v0.1/v0.2:** those versions exposed a required
> `clientAuthMethod` parameter supporting both `client_secret_basic` and
> `client_secret_post`. This version removes `clientAuthMethod` entirely and
> always uses `client_secret_basic` — an existing API definition with
> `clientAuthMethod: client_secret_post` will have that parameter silently
> ignored (it is not a recognized parameter here, so it is not rejected at
> policy load either), and authentication to any identity provider that only
> accepts `client_secret_post` will start failing with `502 Bad Gateway`
> after upgrading to this version. There is no migration path within v0.x
> other than not upgrading past v0.2 until `client_secret_post` support
> returns (`clientAuthMethod` comes back as an optional parameter, defaulting
> to `client_secret_basic`, from v0.7 onward).
...
2. The policy retrieves the current access token from a token source built
   ...
   - For `password`, an expired/missing token triggers a fresh `password`
     ...
     `clientId`/`clientSecret` as HTTP Basic auth) — this policy always
