...
> **Security note:** because the key is derived only from API/route identity
> and `grantType`, it does not change when `clientSecret` (or, for the
> password grant, `password`) is rotated on that same API/route. A credential
> rotation is therefore not guaranteed to invalidate a token already cached
> under the old credential — the still-valid cached token continues to be
> served until its own TTL expires, even though it was minted using
> credentials that are no longer configured. This does not let a caller
> presenting the *new* credential succeed with the *old* token (the token
> itself, not the credential, is what's cached and replayed), but it does
> mean rotating for incident-response reasons does not by itself force a
> fresh token fetch. A later version of this policy folds a hash of
> `clientSecret`/`password` (and the other credential-shaping parameters)
> into the cache key specifically to close this gap — see that version's
> docs for details.
